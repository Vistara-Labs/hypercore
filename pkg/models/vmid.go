...
import (
	...
	pkgerrs "vistara-node/pkg/errors"
)
...
func NewVMID(name, namespace, uid string) (*VMID, error) {
	if name == "" {
		return nil, pkgerrs.ErrNameRequired
	}
...
func splitVMIDFromString(id string) (namespace, name, uid string, err error) {
	...
	if len(parts) != numPartsForID {
		return "", "", "", pkgerrs.IncorrectVMIDFormatError{ActualID: id}
	}

	if parts[0] == "" {
		return "", "", "", pkgerrs.ErrNamespaceRequired
	}

	if parts[1] == "" {
		return "", "", "", pkgerrs.ErrNameRequired
	}

	if parts[2] == "" {
		return "", "", "", pkgerrs.ErrUIDRequired
	}
